<#
    ...
    .LINK
    https://github.com/blow-tech/blow-tech
...
# =============================================================================
$dcdiagLanguages = @(
    @{ Pass = "passed test"; Fail = "failed test" }              # English
    @{ Pass = "a réussi le test"; Fail = "a échoué au test" }    # French
    @{ Pass = "bestanden"; Fail = "nicht bestanden" }            # German
    @{ Pass = "superó la prueba"; Fail = "no superó la prueba" } # Spanish
    @{ Pass = "geslaagd voor"; Fail = "mislukt voor" }           # Dutch
    @{ Pass = "superato"; Fail = "non superato" }                # Italian
    @{ Pass = "aprovado"; Fail = "reprovado" }                   # Portuguese
)
# =============================================================================
...
Function New-ServerHealthHTMLTableCell($lineitem, $reportline) {
    ...
    if ($null -eq $value) { return "<td class='info'>N/A</td>" }
    ...
    if ($lineitem -eq "Replication Summary (num errors)") {
        if ([int]::TryParse($valueStr, [ref]$null)) {
            ...
            return "<td class='$cls'>$valueStr</td>"
        }
        else {
            return "<td class='info'>$valueStr</td>"
        }
    }
    ...
    if ($lineitem -eq "SYSVOL Replication Method") {
        switch ($valueStr.ToUpper()) {
            "FRS" { return "<td class='warn'>FRS</td>" }
            "DFSR" { return "<td class='pass'>DFSR</td>" }
            default { return "<td class='info'>$valueStr</td>" }
        }
    }

    switch ($valueStr) {
        "passed" { return "<td class='pass'>Passed</td>" }
        "failed" { return "<td class='fail'>Failed</td>" }
        "warning" { return "<td class='warn'>Warning</td>" }
        "not installed" { return "<td class='info'>Not Installed</td>" }
        "n/a" { return "<td class='info'>N/A</td>" }
        "no data" { return "<td class='info'>No Data</td>" }
        "unreachable" { return "<td class='info'>Unreachable</td>" }
        "cim error" { return "<td class='fail'>CIM Error</td>" }
        "error" { return "<td class='fail'>Error</td>" }
        "timeout" { return "<td class='info'>Timeout</td>" }
    }
    ...
    if ($lineitem -eq "NTP Source") {
        ...
        if ($lower -eq "unreachable") { return "<td class='info'>Unreachable</td>" }
        if ($lower -eq "fail") { return "<td class='fail'>Fail</td>" }
        if ($lower -match "local cmos|free-running") { return "<td class='fail'>$valueStr</td>" }
        if ($lower -match "vm ic time") { return "<td class='warn'>$valueStr</td>" }
        return "<td class='pass'>$valueStr</td>"
    }
    ...
    if ($lineitem -eq "Tombstone Lifetime") {
        ...
        elseif ($valueStr -like "Error*") {
            return "<td class='fail'>$valueStr</td>"
        }
        else {
...
Function Get-ReplicationDelays($ComputerName, $IsReachable) {
    ...
    try {
        $repl = repadmin /showrepl $ComputerName /csv | ConvertFrom-Csv
        $delays = $repl | Where-Object { $_.'Last Success Time' -ne "" } | ForEach-Object {
            $lastSuccess = [datetime]::Parse($_.'Last Success Time')
            [math]::Round(((Get-Date) - $lastSuccess).TotalHours, 1)
        }
...
H2 { font-size: 16px; }
H3 { font-size: 14px; }
TABLE { border: 1px solid #ccc; border-collapse: collapse; font-size: 10pt; table-layout: fixed; width: 100%; }
TH {
...
<body>
<h1 align="left">Active Directory Health Check Report</h1>
...
<tr>
    ...
    <td class='$forestClass'>$forestLevel</td>
</tr>
<tr>
    <td>Domain Functional Level</td>
    <td class='$domainClass'>$domainLevel</td>
</tr>
</table>
<br>
"@
...
$legend = @"
<br><br>
<h3>Color Legend - AD Health Report</h3>
<table style='border-collapse: collapse; width: 100%; font-size: 9pt;'>
  <tr>
    <th style='text-align: left;'>Field</th>
    <th style='background-color:#6BBF59;'>Green (pass)</th>
    <th style='background-color:#FFD966;'>Yellow (warn)</th>
    <th style='background-color:#D9534F; color:white;'>Red (fail)</th>
    <th style='background-color:#5BC0DE;'>Blue (info)</th>
  </tr>
...
$htmlreport = $htmlhead + $htmlFunctionalLevels + $serverhealthhtmltable + $legend + $htmltail
